	...
	public CodeSystem processCodeSystem(String codeFile, String metaFile, String version, String outFolder)
		...
		for (String line : Utility.readTxtFile(metaFile, false)) {
			...
			if (line.length() > 0) {
				...
				String sex = line.substring(191,192).trim();
		...
		codeSystem.addProperty(
				new PropertyComponent().setCode("Operation name 3 char").setDescription("Operation name 3 char").setType(PropertyType.STRING));
		
		codeSystem.addProperty(
				new PropertyComponent().setCode("Operation name 4 char").setDescription("Operation name 4 char").setType(PropertyType.STRING));
		
		codeSystem.addProperty(
				new PropertyComponent().setCode("Sex").setDescription("Sex").setType(PropertyType.STRING));
		...
		for (Map.Entry<String, OPCSCode> entry : allCodes.entrySet()) {
			...
			if(code.operationName3!=null) {
				concept.addProperty(
						new ConceptPropertyComponent(new CodeType("Operation name 3 char"), new StringType().setValue(code.operationName3)));	
			}
			if(code.operationName4!=null) {
				concept.addProperty(
						new ConceptPropertyComponent(new CodeType("Operation name 4 char"), new StringType().setValue(code.operationName4)));	
			}
			if(code.sexScrutiny!=null) {
				concept.addProperty(
						new ConceptPropertyComponent(new CodeType("Sex"), new StringType().setValue(code.sexScrutiny)));	
			}
